...
use ghostdeck_control::request_io::MAX_REQUEST_LEN;
use ghostdeck_control::socket_path::SocketMode;
use ghostdeck_control::{server, Dispatcher};
...
async fn socket_roundtrip_for_ping() {
    ...
    let socket_path = temp_dir.path().join("ghostdeck-control.sock");
