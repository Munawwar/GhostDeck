...
#[derive(Debug, Parser)]
#[command(
    name = "ghostdeck-control-server",
    about = "ghostdeck control-plane unix socket server"
)]
struct Args {
    #[arg(long)]
...
async fn main() -> anyhow::Result<()> {
    ...
    if std::env::var("GHOSTDECK_DEBUG_LOG")
        .ok()
        .as_deref()
        .map(str::trim)
        .unwrap_or("")
        .is_empty()
    {
        if let Some(log_path) = derive_debug_log_path(&socket) {
            std::env::set_var("GHOSTDECK_DEBUG_LOG", &log_path);
    ...
    let dispatcher = ghostdeck_control::Dispatcher::new();
    ghostdeck_control::server::run_server(&socket, socket_mode, dispatcher).await?;
    Ok(())
}
