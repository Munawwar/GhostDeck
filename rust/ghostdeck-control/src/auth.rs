...
/// Access policy for the GhostDeck Unix control socket.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SocketControlMode {
    /// Allow only descendant processes of the GhostDeck server from the same user.
    GhostDeckOnly,
...
impl SocketControlMode {
    pub fn from_env() -> Self {
        std::env::var("GHOSTDECK_SOCKET_MODE")
            .ok()
    ...
    pub fn requires_owner_only_socket(self) -> bool {
        matches!(self, Self::GhostDeckOnly | Self::LocalUser)
    }
}
...
pub fn is_authorized(peer: &PeerInfo, mode: SocketControlMode) -> bool {
    match mode {
        ...
        SocketControlMode::GhostDeckOnly => peer.uid == current_uid() && is_descendant(peer.pid),
...
mod tests {
    ...
    #[test]
    fn ghostdeck_only_allows_current_process() {
        let peer = PeerInfo {
            pid: std::process::id(),
            uid: current_uid(),
            gid: unsafe { libc::getgid() },
        };
        assert!(is_authorized(&peer, SocketControlMode::GhostDeckOnly));
    }

    #[test]
    fn ghostdeck_only_rejects_non_descendant_pid() {
        let peer = PeerInfo {
            pid: 1,
            uid: current_uid(),
            gid: unsafe { libc::getgid() },
