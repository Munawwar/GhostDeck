...
const GHOSTDECK_SOCKET_ENV: &str = "GHOSTDECK_SOCKET";
const GHOSTDECK_SOCKET_PATH_ENV: &str = "GHOSTDECK_SOCKET_PATH";
const RUNTIME_SUBDIR: &str = "ghostdeck";
const RUNTIME_SOCKET_NAME: &str = "ghostdeck.sock";
const FALLBACK_RUNTIME_SOCKET: &str = "/tmp/ghostdeck.sock";
const DEBUG_SOCKET: &str = "/tmp/ghostdeck-debug.sock";
...
pub fn resolve_socket_path(explicit: Option<PathBuf>, mode: SocketMode) -> PathBuf {
    ...
    if let Some(path) = get_env_path(GHOSTDECK_SOCKET_ENV) {
        return path;
    }
    if let Some(path) = get_env_path(GHOSTDECK_SOCKET_PATH_ENV) {
        return path;
    }
...
mod tests {
    ...
    fn ghostdeck_socket_has_higher_precedence_than_ghostdeck_socket_path() {
        ...
        assert_eq!(resolved, PathBuf::from("/tmp/from-ghostdeck-socket.sock"));
    }

    #[test]
    fn ghostdeck_socket_path_used_when_ghostdeck_socket_missing() {
        let _lock = ENV_TEST_LOCK.lock().expect("env test lock");
        let _socket = EnvGuard::set(GHOSTDECK_SOCKET_ENV, None);
        let _socket_path = EnvGuard::set(
            GHOSTDECK_SOCKET_PATH_ENV,
            Some("/tmp/from-ghostdeck-socket-path.sock"),
        );
    ...
    fn runtime_mode_defaults_to_xdg_runtime_dir() {
        let _lock = ENV_TEST_LOCK.lock().expect("env test lock");
        let _socket = EnvGuard::set(GHOSTDECK_SOCKET_ENV, None);
        let _socket_path = EnvGuard::set(GHOSTDECK_SOCKET_PATH_ENV, None);
    ...
    fn finalize_socket_permissions_sets_socket_mode() {
        ...
        let socket_path = temp_dir.path().join("ghostdeck.sock");
