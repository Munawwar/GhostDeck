...
fn fallback_ghostty_resources_dir() -> std::path::PathBuf {
    let root = std::env::temp_dir().join("ghostdeck-ghostty-resources");
    ...
    if !terminfo_entry.exists() {
        let _ = std::fs::write(
            &terminfo_entry,
            "xterm-ghostty|ghostdeck mock entry\n\tam,\n\tcols#80, lines#24,\n",
        );
    }
    root
}
...
fn run_terminal_command(
    ...
    shell.env("GHOSTDECK_SURFACE_ID", encode_handle_id(surface_id));
...
fn debug_log_path() -> std::path::PathBuf {
    if let Ok(path) = std::env::var("GHOSTDECK_DEBUG_LOG") {
        let trimmed = path.trim();
        if !trimmed.is_empty() {
            return std::path::PathBuf::from(trimmed);
        }
    }
    std::path::PathBuf::from("/tmp/ghostdeck-debug.log")
}
...
fn handle_debug_command(
    ...
    match method {
        ...
        "debug.panel_snapshot" => {
            ...
            let path = write_mock_png("ghostdeck-panel", &label, surface_id)
        ...
        "debug.window.screenshot" => {
            ...
            let path = write_mock_png("ghostdeck-window", &label, screenshot_id)
...
fn handle_command(
    ...
    match method {
        ...
        "surface.create" => {
            ...
            let workspace_id = optional_u64_param_any(params, &["workspace_id"])?;
            let title = optional_string_param(params, "title")?;
            if optional_string_param(params, "type")?
                .is_some_and(|panel_type| panel_type != "terminal")
                || optional_string_param(params, "url")?.is_some()
            {
                return Err(CommandError::invalid_params(
                    "surface.create supports terminals only",
                ));
            }
            with_workspace_scope(state, workspace_id, |scoped| {
                let surface = scoped
...
mod tests {
    ...
    async fn dispatcher_handles_system_commands() {
        ...
        assert_eq!(identify_result["name"], "ghostdeck-control");
    ...
    #[test]
    fn osc9_desktop_notifications_become_ghostdeck_notifications() {
    ...
    #[test]
    fn osc9_control_byte_notifications_become_ghostdeck_notifications() {
        let mut state = ControlState::default();
        let workspace_id = state.current_workspace_id;
        let surface_id = state.workspaces[0].windows[0].panes[0].surfaces[0].id;
