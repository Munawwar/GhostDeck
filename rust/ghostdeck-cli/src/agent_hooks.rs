...
pub(crate) fn launch_record_from_env(
    ...
    let args = std::env::var("GHOSTDECK_AGENT_LAUNCH_ARGV")
        .ok()
    ...
    let executable = std::env::var("GHOSTDECK_AGENT_LAUNCH_EXECUTABLE")
        .ok()
    ...
    Some(AgentLaunchCommandRecord {
        executable,
        arguments: sanitized,
        cwd: std::env::var("GHOSTDECK_AGENT_LAUNCH_CWD")
            .ok()
            .and_then(|value| normalized(&value))
...
fn state_dir() -> PathBuf {
    if let Some(dir) = dirs::state_dir() {
        return dir.join("ghostdeck");
    }
    if let Some(home) = dirs::home_dir() {
        return home.join(".local/state/ghostdeck");
    }
    PathBuf::from(".ghostdeck")
}
