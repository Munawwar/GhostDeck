...
pub const PERSISTENCE_DIR_NAME: &str = "ghostdeck";
...
fn agent_hook_state_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("GHOSTDECK_AGENT_HOOK_STATE_DIR") {
...
fn wrap_restored_agent_command(
    ...
    let cleanup = format!(
        ...
        shell_single_quote(&ghostdeck_cli_executable()),
        kind.store_name()
    );
    format!("{run_command}; ghostdeck_agent_status=$?; {cleanup}; exec \"${{SHELL:-/bin/sh}}\" -l")
}

fn ghostdeck_cli_executable() -> String {
    std::env::current_exe()
        .ok()
        .and_then(|path| {
            ghostdeck_cli_candidates(&path)
                .into_iter()
                .find(|candidate| candidate.exists())
        })
        .map(|path| path.to_string_lossy().to_string())
        .unwrap_or_else(|| "ghostdeck".to_string())
}

fn ghostdeck_cli_candidates(exe: &Path) -> Vec<PathBuf> {
    let mut candidates = Vec::new();

    if let Some(dir) = exe.parent() {
        candidates.push(dir.join("ghostdeck-cli"));

        if let Some(libexec_dir) = dir.parent() {
            if let Some(prefix) = libexec_dir.parent() {
                candidates.push(prefix.join("bin/ghostdeck"));
            }
        }
    }

    candidates.push(PathBuf::from("ghostdeck"));
    candidates
}
...
mod tests {
    ...
    fn persistence_dir_uses_xdg_data_home_directly() {
        ...
        let _xdg = EnvGuard::set("XDG_DATA_HOME", Some("/tmp/ghostdeck-xdg-data"));
        let _home = EnvGuard::set("HOME", Some("/tmp/ghostdeck-home"));

        assert_eq!(
            persistence_dir(),
            PathBuf::from("/tmp/ghostdeck-xdg-data").join(PERSISTENCE_DIR_NAME)
        );
    }
    ...
    fn persistence_dir_falls_back_to_home_local_share_when_data_dir_missing() {
        ...
        assert_eq!(
            persistence_dir(),
            PathBuf::from("/tmp/ghostdeck-home/.local/share").join(PERSISTENCE_DIR_NAME)
        );
    }

    #[test]
    fn ghostdeck_cli_candidates_cover_installed_and_dev_layouts() {
        let installed = Path::new("/usr/libexec/ghostdeck/ghostdeck-host");
        let candidates = ghostdeck_cli_candidates(installed);
        assert!(candidates.contains(&PathBuf::from("/usr/bin/ghostdeck")));

        let dev = Path::new("/repo/target/debug/ghostdeck");
        let candidates = ghostdeck_cli_candidates(dev);
        assert!(candidates.contains(&PathBuf::from("/repo/target/debug/ghostdeck-cli")));
        assert!(candidates.contains(&PathBuf::from("ghostdeck")));
    }
