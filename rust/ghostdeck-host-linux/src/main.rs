...
pub(crate) const APP_ID: &str = "dev.ghostdeck.linux";
...
fn ghostty_resources_candidates(exe_dir: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in exe_dir.ancestors() {
        candidates.push(ancestor.join("share/ghostdeck/ghostty"));
...
fn sanitize_terminal_child_env() {
    // GhostDeck is often launched from another TUI agent session. NO_COLOR belongs
...
fn main() {
    ...
    // terminfo, and shell integration. Prefer GhostDeck-bundled resources but
...
mod tests {
    ...
    fn temp_path(label: &str) -> PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("ghostdeck-{label}-{}-{nanos}", std::process::id()))
    }
    ...
    fn resolves_app_specific_bundled_resources_next_to_executable() {
        ...
        let themes_dir = root.join("share/ghostdeck/ghostty/themes");
        let shell_integration_dir = root.join("share/ghostdeck/ghostty/shell-integration");
        let terminfo_file = root.join("share/ghostdeck/terminfo/g/ghostty");
        ...
        let exe = exe_dir.join("ghostdeck");
        let resolved = resolve_ghostty_resources_dir(&exe).unwrap();
        assert_eq!(resolved, root.join("share/ghostdeck/ghostty"));
    ...
    fn derives_terminfo_dir_from_resources_dir() {
        let resources_dir = PathBuf::from("/usr/share/ghostdeck/ghostty");
        assert_eq!(
            ghostty_terminfo_dir(&resources_dir),
            Some(PathBuf::from("/usr/share/ghostdeck/terminfo"))
        );
    }
    ...
    fn replaces_invalid_inherited_runtime_env_with_resolved_paths() {
        with_ghostty_env(|| {
            ...
            std::env::set_var("GHOSTTY_RESOURCES_DIR", "/app/share/ghostdeck/ghostty");
            std::env::set_var("TERMINFO", "/app/share/ghostdeck/terminfo");
            std::env::set_var(
                "GHOSTTY_SHELL_INTEGRATION_XDG_DIR",
                "/app/share/ghostdeck/ghostty/shell-integration",
            );
