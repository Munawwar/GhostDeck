...
const SCROLLBAR_EMPTY_CSS_CLASS: &str = "ghostdeck-terminal-scrollbar-empty";
...
pub struct TerminalOptions {
    ...
    /// (e.g. `GHOSTDECK_WORKSPACE_ID`, `GHOSTDECK_SURFACE_ID`, `GHOSTDECK_PANE_ID`, `GHOSTDECK_SOCKET`).
    ///
    ...
    /// to call `ghostdeck identify` first. This is the foundation for the cmux
...
pub fn create_terminal(
    ...
    split_dim_overlay.add_css_class("ghostdeck-terminal-split-dim");
    ...
    scrollbar.add_css_class("ghostdeck-terminal-scrollbar");
    ...
    // On realize: create the Ghostty surface
    {
        ...
        gl_area.connect_realize(move |gl_area| {
            ...
            if let Some(err) = gl_area.error() {
                eprintln!("ghostdeck: GLArea error after make_current: {err}");
                return;
            }
            ...
            if let Some(ref command) = c_startup_command {
                ...
                eprintln!(
                    "ghostdeck: starting restored terminal command={}",
            ...
            if surface.is_null() {
                ...
                eprintln!("ghostdeck: failed to create ghostty surface");
                return;
            }
            unsafe {
...
fn show_clipboard_toast(overlay: &gtk::Overlay) {
    ...
    provider.load_from_data(
        ...
        box.ghostdeck-toast label { color: white; } \
        box.ghostdeck-toast button { \
        ...
        box.ghostdeck-toast button:hover { color: white; }",
    );
    ...
    toast.add_css_class("ghostdeck-toast");
