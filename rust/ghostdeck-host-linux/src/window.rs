...
const SPLIT_RATIO_STATE_KEY: &str = "ghostdeck-split-ratio-state";
...
const WORKSPACE_RENAME_ENTRY_CSS_CLASS: &str = "ghostdeck-ws-rename-entry";
const WORKSPACE_RENAME_ENTRY_CSS_CLASSES: [&str; 2] =
    [HOST_ENTRY_CSS_CLASS, WORKSPACE_RENAME_ENTRY_CSS_CLASS];
pub(crate) const SPLIT_PANE_CSS_CLASS: &str = "ghostdeck-split-pane";
const SIDEBAR_HANDLE_CSS_CLASS: &str = "ghostdeck-sidebar-handle";
const SIDEBAR_HANDLE_CURSOR_NAME: &str = "col-resize";
const SIDEBAR_RESIZE_HANDLE_WIDTH_PX: i32 = 2;

const BASE_CSS: &str = r#"
:root {
    --ghostdeck-host-entry-bg: rgba(255, 255, 255, 0.98);
    --ghostdeck-host-entry-fg: rgba(15, 23, 42, 0.96);
    --ghostdeck-host-entry-border: rgba(15, 23, 42, 0.16);
    --ghostdeck-host-entry-border-focus: rgba(0, 145, 255, 0.72);
    --ghostdeck-host-entry-placeholder: rgba(15, 23, 42, 0.5);
}
@media (prefers-color-scheme: dark) {
    :root {
        --ghostdeck-host-entry-bg: rgba(44, 44, 48, 0.98);
        --ghostdeck-host-entry-fg: rgba(255, 255, 255, 0.96);
        --ghostdeck-host-entry-border: rgba(255, 255, 255, 0.14);
        --ghostdeck-host-entry-border-focus: rgba(0, 145, 255, 0.78);
        --ghostdeck-host-entry-placeholder: rgba(255, 255, 255, 0.48);
    }
}
.ghostdeck-host-entry {
    background-color: var(--ghostdeck-host-entry-bg);
    color: var(--ghostdeck-host-entry-fg);
    border: 1px solid var(--ghostdeck-host-entry-border);
    border-radius: 6px;
    caret-color: currentColor;
}
.ghostdeck-host-entry:focus-within {
    border-color: var(--ghostdeck-host-entry-border-focus);
}
.ghostdeck-host-entry text {
    background-color: transparent;
    color: var(--ghostdeck-host-entry-fg);
}
.ghostdeck-host-entry text placeholder {
    color: var(--ghostdeck-host-entry-placeholder);
}
.ghostdeck-host-entry image {
    color: var(--ghostdeck-host-entry-placeholder);
}
.ghostdeck-sidebar {
...
row:selected .ghostdeck-ws-name {
    color: @window_fg_color;
}
.ghostdeck-ws-star-btn {
...
.ghostdeck-drop-below .ghostdeck-sidebar-row-box {
    border-radius: 0;
    box-shadow: 0 2px 0 0 @accent_bg_color;
}
.ghostdeck-tab-drop-target {
    background-color: alpha(@accent_bg_color, 0.18);
    border-radius: 8px;
}
.ghostdeck-sidebar row:drop(active) {
    box-shadow: none;
}
.ghostdeck-sidebar-title {
...
.ghostdeck-sidebar-btn.ghostdeck-tab-drop-target {
...
row:selected .ghostdeck-ws-path {
    color: alpha(@window_fg_color, 0.5);
}
.ghostdeck-content {
    background-color: @window_bg_color;
}
.ghostdeck-terminal-split-dim {
    background-color: alpha(@window_bg_color, 0.30);
}
.ghostdeck-terminal-scrollbar-empty slider {
    opacity: 0;
}
.ghostdeck-split-pane > separator {
    background-color: alpha(@window_fg_color, 0.16);
    min-width: 2px;
    min-height: 2px;
}
.ghostdeck-split-pane > separator:hover {
    background-color: alpha(@window_fg_color, 0.24);
}
.ghostdeck-sidebar-handle {
    min-width: 2px;
    background-color: alpha(@window_fg_color, 0.08);
}
.ghostdeck-sidebar-handle:hover {
...
pub fn build_window(app: &adw::Application) {
    ...
    // already provides the window chrome, so keep GhostDeck from rendering a
    ...
    stack.add_css_class("ghostdeck-content");
    ...
    let sidebar_title_label = gtk::Label::builder()
        ...
        .margin_start(12)
        .build();
    sidebar_title_label.add_css_class("ghostdeck-sidebar-title");
    ...
    btn_drop.set_preload(true);
    {
        ...
        btn_drop.connect_motion(move |_, _, _| {
            if pane::is_tab_dragging() {
                btn.add_css_class("ghostdeck-tab-drop-target");
            } else {
                btn.add_css_class("ghostdeck-sidebar-btn-trash-hover");
            }
...

    {
        ...
        pane::on_tab_drag_change(move |dragging| {
            if dragging {
                btn.add_css_class("ghostdeck-tab-drag-active");
            } else {
                btn.remove_css_class("ghostdeck-tab-drag-active");
                btn.remove_css_class("ghostdeck-tab-drop-target");
            }
        });
    }
...
fn build_window_css((r, g, b): (u8, u8, u8), background_opacity: f64) -> String {
    ...
    format!(
        "{BASE_CSS}\n\
         .ghostdeck-content, .ghostdeck-terminal-scrollbar {{ background-color: rgba({r}, {g}, {b}, {background_opacity:.3}); }}\n\
         .ghostdeck-terminal-scrollbar, .ghostdeck-terminal-scrollbar > range, .ghostdeck-terminal-scrollbar > range > trough {{ min-width: 7px; margin: 0; padding: 0; border: 0; }}\n\
         .ghostdeck-terminal-scrollbar > range, .ghostdeck-terminal-scrollbar > range > trough {{ background-color: transparent; }}\n\
         .ghostdeck-terminal-scrollbar slider {{ min-width: 5px; margin: 0 1px; border: 0; border-radius: 999px; background-color: alpha(@window_fg_color, 0.35); transition: background-color 120ms ease-out; }}\n\
         .ghostdeck-terminal-scrollbar:hover slider {{ background-color: alpha(@window_fg_color, 0.55); }}\n"
    )
}
...
fn set_workspace_favorite_visual(workspace: &Workspace) {
    ...
    if workspace.favorite {
        workspace
            .favorite_button
            .add_css_class("ghostdeck-ws-star-btn-active");
    } else {
        workspace
            .favorite_button
            .remove_css_class("ghostdeck-ws-star-btn-active");
    }
}
...
fn install_workspace_row_interactions(
    ...
    let drop_handled = Rc::new(Cell::new(false));
    {
        ...
        drop_target.connect_motion(move |_, _x, y| {
            ...
            r.remove_css_class("ghostdeck-drop-above");
            r.remove_css_class("ghostdeck-drop-below");
            r.remove_css_class("ghostdeck-tab-drop-target");
            ...
            match dragged_workspace {
                Some(ref dragged_workspace_id) if dragged_workspace_id != &target_workspace_id => {
                    if y < h / 2.0 {
                        r.add_css_class("ghostdeck-drop-above");
                    } else {
                        r.add_css_class("ghostdeck-drop-below");
                    }
                }
                None => {
                    r.add_css_class("ghostdeck-tab-drop-target");
                }
                _ => {}
            }
        ...
        on_config_changed: Rc::new(
            move |previous: &app_config::AppConfig, updated: &app_config::AppConfig| {
                ...
                if let Err(err) = app_config::save(updated) {
                    ...
                    let detail = format!("Failed to save GhostDeck settings: {err}");
                    eprintln!("ghostdeck: {detail}");
                    show_runtime_error(
                        &state_for_config_changed,
...
/// Find the focused pane widget (a gtk::Box with class ghostdeck-pane-toolbar child)
...
fn find_leaf_focused_pane(state: &State) -> Option<(String, gtk::Widget)> {
    ...
    while let Some(w) = widget {
        if let Some(bx) = w.downcast_ref::<gtk::Box>() {
            ...
            while let Some(c) = child {
                if c.has_css_class("ghostdeck-pane-header") {
                    return Some((ws_id, w));
                }
                child = c.next_sibling();
            }
...
fn spawn_new_instance(state: &State) -> bool {
    let exe = match std::env::current_exe() {
        Ok(exe) => exe,
        Err(err) => {
            let detail = format!("Failed to resolve the current GhostDeck executable: {err}");
            eprintln!("ghostdeck: {detail}");
            show_runtime_error(state, "Failed to open a new GhostDeck instance", &detail);
            return false;
        }
    };
    ...
    match std::process::Command::new(exe).spawn() {
        Ok(_) => true,
        Err(err) => {
            let detail = format!("Failed to launch a new GhostDeck instance: {err}");
            eprintln!("ghostdeck: {detail}");
            show_runtime_error(state, "Failed to open a new GhostDeck instance", &detail);
            false
        }
