# Plan: GhostDeck Host Shortcut Remapping Config
...
- Store user preferences in a dedicated GhostDeck config file under `dirs::config_dir()`, not in Ghostty config and not in `session.json`
...
This plan keeps the shortcut feature first-class in the Linux host while avoiding a third shortcut path. `ghostdeck-core` command-palette shortcut hints remain out of scope for this first implementation and should be treated as a follow-up only if the host-side system stabilizes cleanly.
...
- **depends_on**: []
- **location**: `rust/ghostdeck-host-linux/src/main.rs`, `rust/ghostdeck-host-linux/src/window.rs`, `rust/ghostdeck-host-linux/src/pane.rs`
...
- **status**: Completed
- **log**: `reason_not_testable`: inventory-only task. Verified by direct code inspection. Current GTK-global actions are only `win.new-workspace`, `win.close-workspace`, `win.toggle-sidebar`, `win.next-workspace`, and `win.prev-workspace` in `rust/ghostdeck-host-linux/src/main.rs:103-107`, with matching `gio::SimpleAction` wiring in `rust/ghostdeck-host-linux/src/window.rs:827-849`. Capture-only host shortcuts are implemented in `rust/ghostdeck-host-linux/src/window.rs:864-980`: `new_workspace`, `close_workspace`, `cycle_tab_prev`, `cycle_tab_next`, `split_down`, `new_terminal`, `split_right`, `close_focused_pane`, `toggle_sidebar`, `next_workspace`, `prev_workspace`, `focus_left`, `focus_right`, `focus_up`, `focus_down`, and `activate_workspace_1` through `activate_workspace_9_or_last`. Gotchas for follow-up tasks: `Ctrl+T` and `Ctrl+Shift+T` both dispatch to `add_tab_to_focused_pane(false)` in `rust/ghostdeck-host-linux/src/window.rs:890-913`; only five actions currently exist as `gio::SimpleAction`s; pane action buttons are wired independently in `rust/ghostdeck-host-linux/src/pane.rs:244-278`; and Ghostty terminal input remains the passthrough owner for unmapped keys via `ghostty_surface_key(...)` in `rust/ghostdeck-host-linux/src/terminal.rs:566-610`. UI surfaces with hardcoded shortcut text are the sidebar collapse and expand tooltips in `rust/ghostdeck-host-linux/src/window.rs:623` and `rust/ghostdeck-host-linux/src/window.rs:683`. Pane buttons in `rust/ghostdeck-host-linux/src/pane.rs:190-194` expose action tooltips without shortcut text today and will need registry-backed labels once remapping exists.
- **files edited/created**: `shortcut-remap-plan.md`

### T2: Define Canonical Shortcut Metadata and Dispatch Layer
- **depends_on**: [T1]
- **location**: `rust/ghostdeck-host-linux/src/shortcut_config.rs` (new), `rust/ghostdeck-host-linux/src/window.rs`, `rust/ghostdeck-host-linux/src/main.rs`, `rust/ghostdeck-host-linux/src/pane.rs`
...
- **validation**: `cargo test -p ghostdeck-host-linux` covers the config contract and fails if loader behavior regresses on any supported edge case.
- **status**: Completed
- **log**: Verified existing branch state rather than re-implementing. The targeted suite in `rust/ghostdeck-host-linux/src/shortcut_config.rs` already covers path derivation, normalized shortcut round-trips, override application, explicit unbinding, unknown ID warnings, duplicate-binding rejection, invalid JSON fallback, missing-file defaults, GTK accelerator exposure for unbound actions, and runtime combo-to-command routing. Validation command: `cargo test -p ghostdeck-host-linux shortcut_config::tests -- --nocapture` passed with 12 tests.
- **files edited/created**: `rust/ghostdeck-host-linux/src/shortcut_config.rs`, `shortcut-remap-plan.md`

### T5: Switch GTK Accelerators and Capture-Phase Dispatch to the Same Registry
- **depends_on**: [T4]
- **location**: `rust/ghostdeck-host-linux/src/main.rs`, `rust/ghostdeck-host-linux/src/window.rs`
...
- **status**: Completed
- **log**: RED phase added pure tooltip-contract tests in `rust/ghostdeck-host-linux/src/shortcut_config.rs`, `rust/ghostdeck-host-linux/src/window.rs`, and `rust/ghostdeck-host-linux/src/pane.rs`, then ran `cargo test -p ghostdeck-host-linux tooltip -- --nocapture`, which failed because the display/tooltip helpers and call sites did not exist. GREEN changes added one shared display path in `rust/ghostdeck-host-linux/src/shortcut_config.rs` (`to_display_label`, `display_label_for_id`, and `tooltip_text`), replaced the hardcoded sidebar toggle tooltip strings in `rust/ghostdeck-host-linux/src/window.rs` with `sidebar_toggle_tooltip(...)`, and threaded the resolved shortcut registry into `rust/ghostdeck-host-linux/src/pane.rs` so pane action buttons derive tooltip text from the same registry while unbound actions omit shortcut suffixes. Also removed the unused `find_by_action_name` helper to keep the branch warning-free. GREEN commands: `cargo test -p ghostdeck-host-linux tooltip -- --nocapture`, `cargo test -p ghostdeck-host-linux`, and `cargo build -p ghostdeck-host-linux --features webkit` all passed.
- **files edited/created**: `rust/ghostdeck-host-linux/src/shortcut_config.rs`, `rust/ghostdeck-host-linux/src/window.rs`, `rust/ghostdeck-host-linux/src/pane.rs`, `shortcut-remap-plan.md`
...
## Testing Strategy
- Run `cargo test -p ghostdeck-host-linux`
- Run `cargo build -p ghostdeck-host-linux --features webkit`
...
cargo run -p ghostdeck-host-linux --features webkit --bin ghostdeck
```
...
- `ghostdeck-core` command-palette shortcut hints currently use a separate model.
